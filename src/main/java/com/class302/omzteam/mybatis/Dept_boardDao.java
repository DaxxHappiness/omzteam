...
public interface Dept_boardDao {
    ...
    @Select("SELECT * FROM dept_board where dept_no = #{dept_no} LIMIT #{page}, 10;")
    ...
    @Select("select * FROM dept_board WHERE dept_no = #{dept_no} AND writer LIKE CONCAT('%', #{value}, '%') LIMIT #{page}, 10")
    List<Dept_board> nameselect(int dept_no, String value,int page);

    @Select("select * FROM dept_board WHERE dept_no = #{dept_no} AND title LIKE CONCAT('%', #{value}, '%') LIMIT #{page}, 10")
    List<Dept_board> titleselect(int dept_no,String value,int page);

    @Select("select * FROM dept_board WHERE dept_no = #{dept_no} ORDER BY is_notice desc;")
    List<Dept_board> findDbpard(int dept_no);
